...
import com.ferrer.johnoliver.block1.project.soriesthriftshop.databinding.FragmentFavouritesBinding

class FavouritesFragment : Fragment() {

    private lateinit var favouritesItemList: LinearLayout
    ...
    override fun onCreateView(
        ...
        favouritesItemList = binding.favouritesItemList // Reference the LinearLayout
        setupFavouritesObserver(favouritesItemList) // Pass the favouritesItemList LinearLayout as the parent ViewGroup here
        return binding.root
    }

    private fun setupFavouritesObserver(parent: ViewGroup) {
        favouritesViewModel.favouriteItems.observe(viewLifecycleOwner) { items ->
            // Clear the existing views before adding new ones
            favouritesItemList.removeAllViews()
            items.forEach { item ->
                val itemView = LayoutInflater.from(context).inflate(R.layout.favourite_item_layout, parent, false)
                ...
                favouritesItemList.addView(itemView)
            }
        }
    }
